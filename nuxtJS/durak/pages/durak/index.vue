<template>
  <div class="view__durak">
    <div class="mdl-card mdl-shadow--2dp">
      <button
        @click="randomizeCards(wildCardsArray)"
      ...
      <div class="dice-animation" :class="diceClasses">
        ...
        <div class="wild-card" v-for="(card, index) in randomCards">
          <i class="material-icons mdl-list__item-icon">filter_{{index + 1}}</i>
          {{ card }}
        </div>
      </div>
    </div>
    ...
    <div class="mdl-card mdl-shadow--2dp">
      <button
        @click="randomizeCardsNachzieh(wildCardsNachziehArray)"
        class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent"
      >
        <i class="material-icons">casino</i>
        Nachzieh Wildcards
    ...
    <div class="mdl-card mdl-shadow--2dp dice-box">
      <button
        @click="rollTheDice(6)"
        class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent"
      >
      ...
      <div class="roll-dice-animation x--nachzieh" :class="nachziehClasses">
        <div class="roll-the-dice" :class="rollDiceClasses"></div>
...
export default {
  ...
  computed: {
    diceClasses() {
      return [{ [`x--roll`]: this.randomCards.length > 0 }];
    },
    nachziehClasses() {
      return [{ [`x--roll`]: this.wildCardsNachzieh.length > 0 }];
    },
    rollDiceClasses() {
      return [{ [`x--roll-dice`]: this.showDice }];
    }
  },
  methods: {
...
.dice-animation {
  ...
  &.x--roll {
    ...
    &.x--nachzieh {
      .silicon-valley {
        opacity: 1;
        animation: dice 0.2s ease 4.3s forwards;
      }
    }
  }
...
.dice-box {
  ...
  .roll-the-dice {
    ...
    width: 100%;
    background-size: 100% auto;
    background-image: url("../../assets/roll-the-dice.gif");
    display: none;
    ...
    &.x--roll-dice {
      display: block;
      animation: rollDice ease 3s forwards;
    }
  }
